...
def generate_role(role_profile: str) -> str:
    """ 用chatglm生成人設描写 """

    instruction = f"""
请从下列文本中，抽取人設描写。若文本中不包含人設，请你推测人物的基本設定：姓名、年齡、身高、出生背景成長背景設定等。要求：
1. 只生成人設描写，不要生成任何多余的内容。
2. 人設描写不能包含敏感词，人物形象需得体。
3. 尽量用短语描写，而不是完整的句子。
4. 不要超过50字
...
"""
    ...
    # 获取生成器
    response_generator = get_chatglm_response_via_sdk(
        messages=[
            {
                "role": "user",
                "content": instruction.strip()
    ...
    # 将生成器中的所有响应片段合并为一个字符串
    full_description = ''.join(list(response_generator))
